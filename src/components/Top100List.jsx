...
export default function Top100List() {
  // Aggregated list across paginated responses
  const [media, setMedia] = useState([]);
  const [page, setPage] = useState(1);
  // Separate flags for initial load vs incremental load
  ...
  useEffect(() => {
    const fetchTopManga = async () => {
      ...
      try {
        ...
        // Basic retry if Jikan rate-limits this request
  ...
  if (isLoading && page === 1)
    return (
      <div className="animate-pulse bg-zinc-900/30 h-96 rounded-xl mt-8 mb-10"></div>
    );
  ...
  return (
    <div className="mt-8 mb-10">
      ...
      <div className="flex flex-col gap-3">
        {media.map((manga, index) => (
          <div
            ...
            className="flex items-center bg-zinc-900/40 hover:bg-zinc-800/80 transition rounded-lg p-3 gap-4 border border-zinc-800/50"
          >
            ...
            <div className="flex-grow flex flex-col justify-center">
              <h3 className="font-semibold text-zinc-200 text-sm md:text-base line-clamp-1">
                {manga.title}
              </h3>
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {manga.genres.slice(0, 3).map((genre) => (
                  <span
                    key={genre.mal_id}
                    className="text-[10px] bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded-full font-medium tracking-wide"
                  >
            ...
            <div className="hidden md:flex flex-col items-end justify-center w-28">
              ...
              <div className="text-xs text-zinc-500 mt-0.5">
                {manga.scored_by?.toLocaleString()} reviews
              </div>
            </div>
      ...
      {media.length < 100 && (
        <button
          // Keep loading pages until we hit the top-100 target
